...
The repository contains an executable fixture shell and BFF, the stateless Zenith membership issuer, and the Web verifier. `/start` supports either extension-owned Wallet UI or an encrypted browser wallet intended for mobile use. Both derive the same Dregg-compatible Ed25519 + ML-DSA-65 identity, prove possession of the Ed25519 Member Key, and independently verify the deterministic Zenith-signed credential. There is no individual/institution selector, application, review, Control call, cookie, or server sign-in session. Dregg permissionless v2 remains preserved as a dormant forward-compatible path. Once a browser wallet or extension exposes a usable keypair, the primary **Join** action becomes **My Castalia** at `/my-castalia`; membership issuance is a separate status and `/profile` remains a compatibility route. The browser-wallet page is an early-Web-style private personal home and editor for self-asserted Person, Author, Researcher, Student, academic-institution, and journal claims kept in an encrypted `.castaway` identity section; disclosure is an explicit selected-claims export. `/chronicle` publishes the checked-in essay on portable data. The [Castaway vault boundary](docs/castaway-portable-vault.md) remains separate from narrow signing-authority recovery. See the [mobile Web wallet boundary](docs/web-wallet-mobile.md), [membership onboarding boundary](docs/membership-onboarding.md), [product boundary](docs/product-boundary.md), [claim ledger](docs/authority-and-claims.md), and [Castalia Control authority boundary](docs/castalia-control-authority.md).
...
- `crates/castalia-wallet-wasm` — Rust/WASM validation plus encrypted browser custody. The custody module generates one random Dregg-hybrid identity root and signs only inside a dedicated Worker; legacy membership-v2 validation remains compatibility code.
