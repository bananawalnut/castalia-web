...
| S8 | High | A browser self-digest did not make an analysis authoritative. | Browser output is labelled person-scoped WMT evidence; canonicalized, domain-separated, locked-CI regeneration may verify its bytes and computation but never promote it to objective or decision authority. |
...
| ID | Severity | Finding | Resolution in current draft |
| --- | --- | --- | --- |
| P1 | High | “Disagreement,” “position,” “forced consequence,” and WMT “authority” language could masquerade as truth, consensus, or social fact. | WMT evidence is now explicitly subjective to the attributed person's bucket and named bucket version. Consistency-gated personal insertion and any RFC-scoped bucket remain separate; none is truth, consensus, peer review, or decision authority. |
