...
- Added an opt-in mobile browser wallet on `/start` with Worker-owned Dregg-hybrid custody, IndexedDB persistence, plaintext recovery-key and encrypted `.castalia-recovery` restore paths, recovery-before-issuance gating, and strict Zenith credential verification.
